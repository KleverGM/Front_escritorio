...
export default function PrivateLayout({ onLogout }: { onLogout: () => void }) {
  return (
    <div className="min-h-screen grid grid-cols-1 md:grid-cols-[320px_1fr] bg-gray-50 text-slate-900">
...
function SearchControl() {
  ...
  return (
    <form onSubmit={submitSearch} className="flex items-center">
      ...
      />
      <button type="submit" className="ml-2 px-3 py-1 bg-[#f8b31d] rounded text-black font-medium">Buscar</button>
      <button type="button" onClick={() => setQ("")} className="ml-2 text-sm text-slate-600">Cancelar</button>
    </form>
  );
}
